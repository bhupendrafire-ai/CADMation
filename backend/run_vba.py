...
def vba_measure_all():
    ...
    macro_path = os.path.abspath("measure_macro.catvbs")
    out_path = os.path.abspath("macro_out.txt")
    
    vba = f"""
Sub CATMain()
    On Error Resume Next
    Dim outPath
    outPath = "{out_path.replace('\\', '\\\\')}"
...
"""
    ...
    try:
        # Evaluate doesn't work well with multiline, let's use ExecuteScript
        from win32com.client import Dispatch
        # system service can execute script from file
        caa.SystemService.ExecuteScript(os.path.dirname(macro_path), 1, "measure_macro.catvbs", "CATMain", [])
        
        if os.path.exists(out_path):
            with open(out_path, "r") as f:
                print(f.read())
        else:
            print("Macro output file not found")
